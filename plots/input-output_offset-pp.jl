...
ω_c1 = Observable(2170.0)  # pump cavity mode frequency
ω_c2 = Observable(2170.0)  # probe cavity mode frequency
g_1 = Observable(25 / √(N))
g_3 = Observable(0.2)
Δ = Observable(20.0)  # anharmonicity: ω_12 = ω_0 - 2Δ
ω_12 = @lift($ω_0 - 2 * $Δ)  # v=1 to v=2 transition

Toff = @lift(transmission(ω, 0, $(ω_0), $(ω_c2), $(Δ), γ_m, κ, $(g_1), $(g_3), N))
Ton = @lift(transmission(ω, f_pu, $(ω_0), $(ω_c1), $(Δ), γ_m, κ, $(g_1), $(g_3), N))
...
sg = SliderGrid(
    fig[2, 1][2, 1],
    (label = L"ω_c (pump)", range = 2100:0.1:2300, format = "{:} cm⁻¹", startvalue = to_value(ω_0)),
    (label = L"ω_c (probe)", range = 2100:0.1:2300, format = "{:} cm⁻¹", startvalue = to_value(ω_0)),
    (label = L"g_1", range = 5:0.1:50, format = "{:.1f} cm⁻¹", startvalue = 20.0),
    (label = L"\frac{g_3}{g_1}", range = -1:0.01:1, format = "{:.1f}", startvalue = 0.0),
...
connect!(ω_c1, sliderobservables[1])
connect!(ω_c2, sliderobservables[2])
connect!(g_1, sliderobservables[3])
connect!(g_3, sliderobservables[4])
connect!(Δ, sliderobservables[5])
